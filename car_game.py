...
class Car():
    def __init__(self, x=0, y=0, dx=0, dy=0):
        ...
        collision_sound_path = resource_path('assets/crash.wav')
        self.collision_sound = pygame.mixer.Sound(collision_sound_path)

        engine_sound_path = resource_path('assets/collision.wav')
    ...
    def out_of_screen(self):
        ...
        elif self.y < 0 or self.y > (SCREEN_HEIGHT - self.height):
             self.y -= self.dy
...
class Game():
    ...
    def draw_text(self, screen, text, font, x, y, main_color):
        text_obg = font.render(text, True, main_color)
        text_rect = text_obg.get_rect()
        text_rect.center = x, y
        screen.blit(text_obg, text_rect)
